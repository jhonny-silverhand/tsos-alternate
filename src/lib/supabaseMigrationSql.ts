/**
 * Complete standalone SQL migration script for Supabase.
...
-- Anti-tamper Table Verification RPC
...
BEGIN
  ...
  FROM public.locations WHERE slug = p_cafe_slug;

  IF NOT FOUND THEN
    RETURN jsonb_build_object('valid', FALSE, 'error', 'Cafe not found');
  END IF;

  SELECT id, label, qr_token INTO v_table
  FROM public.dining_tables
  WHERE location_id = v_location.id AND LOWER(label) = LOWER(p_table_label);

  IF NOT FOUND THEN
    RETURN jsonb_build_object('valid', FALSE, 'error', 'Table not found');
  END IF;

  IF v_table.qr_token <> p_token THEN
    RETURN jsonb_build_object('valid', FALSE, 'error', 'Anti-Tamper: Token mismatch for table');
  END IF;
